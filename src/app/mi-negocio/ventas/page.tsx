...
import { ArrowLeft, Loader2, Printer, ReceiptText, RefreshCw, Trash2, X } from "lucide-react";
...
export default function VentasPage() {
  ...
  return <div className="pb-24">
    ...
    <Dialog open={selected !== null} onClose={() => setSelected(null)} labelledBy="sale-detail-title" panelClassName="max-w-xl rounded-2xl border border-stampa-border bg-stampa-surface">
      {selected && <><div className="flex items-start justify-between gap-4 border-b border-stampa-border p-5"><div><h2 id="sale-detail-title" className="text-lg font-bold text-white">Venta N.º {selected.sale_number}</h2><p className="mt-1 text-sm text-gray-500">{dateTime.format(new Date(selected.created_at))} · {selected.client_name ?? "Sin cliente"}</p></div><button type="button" onClick={() => setSelected(null)} className="rounded-lg p-2 text-gray-500 hover:bg-white/5"><X size={18} /></button></div><div className="divide-y divide-stampa-border">{selected.items.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 p-4"><div className="min-w-0"><p className="truncate text-sm font-bold text-white">{item.product_name_snapshot}</p><p className="text-xs text-gray-500">{item.quantity} × {money.format(item.unit_price)}</p></div><p className="shrink-0 text-sm font-bold text-white">{money.format(item.subtotal)}</p></div>)}</div>
      <div className="border-t border-stampa-border px-5 py-3 text-xs text-gray-400">
        <p className="font-bold uppercase tracking-wide text-gray-500">Pago</p>
        <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1">
          {selected.payments.map((payment) => <span key={payment.id}>{payment.method === "cash" ? "Efectivo" : "Transferencia"}: {money.format(payment.amount)}</span>)}
          {selected.debtAmount > 0 && <span className="font-bold text-amber-300">Cuenta corriente: {money.format(selected.debtAmount)}</span>}
        </div>
      </div>
      {selected.status === "voided" && <div className="border-t border-red-500/20 bg-red-500/5 px-5 py-3 text-xs text-red-200">Anulada{selected.void_reason ? ` · ${selected.void_reason}` : ""}</div>}<div className="flex items-end justify-between border-t border-stampa-border p-5"><div><p className="text-xs uppercase tracking-wide text-gray-500">Estado</p><p className={`mt-1 text-sm font-bold ${selected.status === "completed" ? "text-emerald-300" : "text-red-300"}`}>{selected.status === "completed" ? "Completada" : "Anulada"}</p></div><div className="text-right"><p className="text-xs text-gray-500">Total</p><p className="text-xl font-black text-white">{money.format(selected.total)}</p></div></div><div className="grid gap-2 border-t border-stampa-border p-4"><Link href={`/mi-negocio/ticket/${selected.id}`} target="_blank" className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-stampa-border text-sm font-bold text-gray-200 hover:bg-white/5"><Printer size={16} /> Imprimir ticket</Link><button type="button" disabled={selected.status === "voided" || selected.is_online_sale} onClick={() => setVoidDialogOpen(true)} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-red-500/30 bg-red-500/5 px-4 text-sm font-bold text-red-300 disabled:cursor-not-allowed disabled:opacity-45"><Trash2 size={16} /> {selected.status === "voided" ? "Venta ya eliminada" : "Eliminar venta"}</button>{selected.is_online_sale && selected.status !== "voided" && <p className="mt-1 text-center text-xs leading-5 text-amber-200">Esta venta está asociada a un pedido online y no puede eliminarse desde acá.</p>}</div></>}
    </Dialog>
