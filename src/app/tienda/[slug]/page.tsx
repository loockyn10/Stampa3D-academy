...
import { loadPublicStorefront, loadPublicStorefrontCheckoutStatus, loadPublicStorefrontProducts } from "@/lib/business/storefront";
...
export default async function PublicStorefrontPage({ params }: StorePageProps) {
  const { slug } = await params;
  const supabase = await createClient();
  const [store, products, checkout] = await Promise.all([loadPublicStorefront(supabase, slug), loadPublicStorefrontProducts(supabase, slug), loadPublicStorefrontCheckoutStatus(supabase, slug)]);
  if (!store) notFound();
