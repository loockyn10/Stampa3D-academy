    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div className="bg-stampa-surface p-6 rounded-xl border border-stampa-border shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          {editingCatId === "new" && (
            <div className="p-3 bg-stampa-bg-soft rounded-lg border border-stampa-border space-y-3">
              <input type="text" placeholder="Nombre" value={catForm.name} onChange={e => setCatForm({...catForm, name: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
              <input type="text" placeholder="Slug" value={catForm.slug} onChange={e => setCatForm({...catForm, slug: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
              <input type="number" placeholder="Orden" value={catForm.sort_order} onChange={e => setCatForm({...catForm, sort_order: parseInt(e.target.value) || 0})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
          ...
          {categories.map((cat) => (
            <div key={cat.id}>
              {editingCatId === cat.id ? (
                <div className="p-3 bg-stampa-bg-soft rounded-lg border border-blue-200 space-y-3">
                  <input type="text" value={catForm.name} onChange={e => setCatForm({...catForm, name: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
                  <input type="text" value={catForm.slug} onChange={e => setCatForm({...catForm, slug: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
                  <input type="number" value={catForm.sort_order} onChange={e => setCatForm({...catForm, sort_order: parseInt(e.target.value) || 0})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
                ...
                <div className="flex items-center justify-between p-3 border border-stampa-border rounded-lg hover:bg-stampa-bg-soft">
                  <div>
      ...
      <div className="bg-stampa-surface p-6 rounded-xl border border-stampa-border shadow-sm space-y-4">
        ...
        <div className="space-y-3">
          {editingInstId === "new" && (
            <div className="p-3 bg-stampa-bg-soft rounded-lg border border-stampa-border space-y-3">
              <input type="text" placeholder="Nombre" value={instForm.name} onChange={e => setInstForm({...instForm, name: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
              <input type="text" placeholder="Avatar URL" value={instForm.avatar_url} onChange={e => setInstForm({...instForm, avatar_url: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
              <textarea placeholder="Bio" value={instForm.bio} onChange={e => setInstForm({...instForm, bio: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" rows={2} />
              <div className="flex justify-end gap-2">
          ...
          {instructors.map((inst) => (
            <div key={inst.id}>
              {editingInstId === inst.id ? (
                <div className="p-3 bg-stampa-bg-soft rounded-lg border border-blue-200 space-y-3">
                  <input type="text" value={instForm.name} onChange={e => setInstForm({...instForm, name: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
                  <input type="text" placeholder="Avatar URL" value={instForm.avatar_url} onChange={e => setInstForm({...instForm, avatar_url: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" />
                  <textarea value={instForm.bio} onChange={e => setInstForm({...instForm, bio: e.target.value})} className="bg-stampa-surface text-white w-full text-sm border-white/20 rounded-md" rows={2} />
                  <div className="flex items-center gap-2">
