...
export function LandingMetrics() {
  ...
  return (
    <section className="bg-stampa-bg py-12 md:py-20 border-b border-stampa-border relative overflow-hidden" ref={ref}>
      {/* Subtle background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80%] h-full bg-stampa-orange/5 blur-[100px] rounded-full pointer-events-none hidden md:block" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {metrics.map((metric, idx) => (
            <div 
              key={idx} 
              className={`bg-zinc-900/50 border border-stampa-border rounded-2xl p-8 flex flex-col items-center text-center transition-all duration-500 hover:bg-zinc-800/80 hover:border-stampa-orange/30 md:hover:-translate-y-2 hover:shadow-[0_10px_30px_rgba(234,88,12,0.15)] stampa-reveal-hidden ${isIntersecting ? 'stampa-reveal-visible' : ''}`}
