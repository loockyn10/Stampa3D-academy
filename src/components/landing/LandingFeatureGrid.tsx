...
export function LandingFeatureGrid() {
  ...
  return (
    <section id="contenido" className="py-24 bg-stampa-bg text-white relative">
      <div className="container mx-auto px-6" ref={ref}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, idx) => (
            <div 
              key={idx}
              className={`group bg-zinc-900/50 border border-stampa-border rounded-2xl p-8 hover:bg-zinc-900 hover:border-stampa-orange/50 hover:shadow-[0_0_30px_rgba(234,88,12,0.2)] hover:-translate-y-2 transition-all duration-500 stampa-reveal-hidden ${isIntersecting ? 'stampa-reveal-visible' : ''}`}
              style={{ animationDelay: `${idx * 0.1 + 0.2}s` }}
            >
              <div className="w-12 h-12 rounded-xl bg-stampa-orange/10 border border-stampa-orange/20 flex items-center justify-center text-stampa-orange mb-6 group-hover:scale-110 group-hover:bg-stampa-orange/30 transition-all duration-300">
