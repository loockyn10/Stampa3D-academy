      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            ...
            <input type="number" name="total_grams" value={formData.total_grams !== undefined ? formData.total_grams : ""} onChange={handleChange} className="w-full text-sm border-stampa-border rounded-md text-neutral-100 bg-stampa-surface border focus:border-[#ff6a00] focus:ring-[#ff6a00]/20 focus:ring-2 placeholder:text-neutral-500 disabled:bg-neutral-800 disabled:text-neutral-500" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-300 mb-1">Gramos Restantes</label>
            <input type="number" name="remaining_grams" value={formData.remaining_grams !== undefined ? formData.remaining_grams : ""} onChange={handleChange} className="w-full text-sm border-stampa-border rounded-md text-neutral-100 bg-stampa-surface border focus:border-[#ff6a00] focus:ring-[#ff6a00]/20 focus:ring-2 placeholder:text-neutral-500 disabled:bg-neutral-800 disabled:text-neutral-500" />
          </div>
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-300 mb-1">Precio Compra ($)</label>
          <input type="number" name="purchase_price" value={formData.purchase_price !== undefined ? formData.purchase_price : ""} onChange={handleChange} className="w-full text-sm border-stampa-border rounded-md text-neutral-100 bg-stampa-surface border focus:border-[#ff6a00] focus:ring-[#ff6a00]/20 focus:ring-2 placeholder:text-neutral-500 disabled:bg-neutral-800 disabled:text-neutral-500" />
        </div>
        <div className="flex items-center gap-2 pt-1">
          <input type="checkbox" name="is_active" checked={formData.is_active !== undefined ? formData.is_active : true} onChange={handleChange} className="rounded text-stampa-orange focus:ring-[#ff6a00]/20" />
