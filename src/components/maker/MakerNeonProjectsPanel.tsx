...
/** Proyectos de Neon LED: guardar / guardar como / nuevo / abrir / eliminar. Solo presentación: la lógica vive en useNeonProjects. */
export function MakerNeonProjectsPanel({ library }: { library: ReturnType<typeof useNeonProjects> }) {
    ...
    <Card className="flex flex-col gap-3 p-4">
      ...
      {listOpen && (
        <div className="flex max-h-48 flex-col gap-1 overflow-y-auto rounded-xl border border-white/10 bg-white/[0.03] p-1.5">
          ...
          {!projectsLoading && projects.length === 0 && <span className="px-2 py-1 text-xs text-gray-500">Todavía no guardaste proyectos Neon.</span>}
          {projects.map((p) => (
            <div key={p.id} className="flex items-center gap-1">
              ...
              >
                {p.name}
                <span className="ml-2 font-normal text-gray-500">{p.sourceType.replace("neon-", "")}</span>
              </button>
